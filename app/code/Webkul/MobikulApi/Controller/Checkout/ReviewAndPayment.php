    ...
    protected function getPaymentMethods($quote)
    {
        ...
        foreach ($paymentDetails["payment_methods"] as $method) {
            ...
            if (in_array($method->getCode(), ["paypal_standard", "paypal_express"])) {
                ...
                $oneMethod["extraInformation"] = $extraInformation;
            } else if ($method->getCode() == "walletsystem") {
                $type = "";
                foreach ($quote->getAllVisibleItems() as $item) {
                    $eachItem = [];
                    $type= $this->helperCatalog->stripTags($item->getSku());
                }
                if ($type == "wk_wallet_amount") {
                    break;
                }
            }
            ...
            $logger->info("194");
            $logger->info(json_encode($this->returnArray['paymentMethods']));
        }
    }
