<?php
/**
 * Webkul Software.
 *
 *
 * @category  Webkul
 * @package   Webkul_DeliveryBoy
 * @author    Webkul <support@webkul.com>
 * @copyright Webkul Software Private Limited (https://webkul.com)
 * @license   https://store.webkul.com/license.html ASL Licence
 * @link      https://store.webkul.com/license.html
 */
namespace Webkul\DeliveryBoy\Model\Deliveryboy\Validator;

class NotAdminValidator implements \Zend_Validate_Interface
{
    /**
     * @var \Webkul\DeliveryBoy\Helper\Data
     */
    private $helperData;

    /**
     * @param \Webkul\DeliveryBoy\Helper\Data $helperData
     */
    public function __construct(\Webkul\DeliveryBoy\Helper\Data $helperData)
    {
        $this->helperData = $helperData;
    }

    /**
     * Validation error messages
     *
     * @var array
     */
    private $_messages = [];

    /**
     * Check whether the entity is valid according to defined validation rules
     *
     * @param \Magento\Framework\DataObject $entity
     * @return bool
     *
     * @throws \Exception
     * @api
     */
    public function isValid($entity)
    {
        $this->_messages = [];
        if ($this->helperData->getAdminEmail() === $entity->getEmail()) {
            $this->_messages[] = __('A Delivery boy with email ' . $entity->getEmail() . ' already exists.');
        }
        
        return empty($this->_messages);
    }

    /**
     * Return error messages (if any) after the last validation
     *
     * @return array
     */
    public function getMessages()
    {
        return $this->_messages;
    }
}
